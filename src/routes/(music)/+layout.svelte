...
<svelte:head>
    ...
    <meta name="description" content="IDOLY PRIDE Music Database" />
    <meta property="og:url" content="https://idolypride.cyle.me" />
    <meta property="og:title" content="IDOLY MUSICS" />
    <meta property="og:description" content="IDOLY PRIDE Music Database" />
    <meta property="twitter:url" content="https://idolypride.cyle.me" />
    <meta name="twitter:title" content="IDOLY MUSICS" />
    <meta name="twitter:description" content="IDOLY PRIDE Music Database" />
</svelte:head>
...
<div class="flex min-h-screen flex-col bg-base-300">
	...
	<div  id="navibar" class="navbar fixed top-0 z-[100] h-16 w-full bg-base-100 px-3 lg:px-6 shadow-md">
			...
			<div id="left-drawer-wrapper" class="z-[52] flex">
					...
					<div class="drawer-side">
						...
						<ul class="menu min-h-full w-80 bg-base-200 p-4 text-base text-base-content">
							<SiteNavigation onNavigate={(destination) => { if (destination === '/') reset(); isDrawerOpen = false; }} />
						</ul>
					</div>
			</div>
		...
		<div id="change-lang-theme" class="z-[51] flex-none">
			<button class="btn btn-ghost btn-square" aria-label="마나의 일기" title="마나의 일기" on:click={() => { isDiaryOpen = true; isDrawerOpen = false; }}>
				<svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 6c-3-2-6-2-9-1v14c3-1 6-1 9 1m0-14c3-2 6-2 9-1v14c-3-1-6-1-9 1m0-14v14" /></svg>
			</button>
			<LangDrop/>
